...
step::: Retrieve the collection items as described in test xref:/conf/api-common/collection-items[style=id%].
step::: Iterate through all items in the collection and retrieve their type.
step::: Check that the reported type a valid URI or CURIE.
step::: Validate that the contents of the returned document conform to the media type reported by the response `Content-Type` header.
step:::: If the response content type is `{geojson-mediatype}`, execute test xref:/conf/geojson/deployment-schema[style=id%].
step:::: If the response content type is `{sensorml-json-mediatype}`, execute test xref:/conf/sensorml/deployment-schema[style=id%].
step:::: For other response content types not supported by the testing engine, issue a warning and skip this test.
...
====
[%metadata]
identifier:: /conf/subdeployment/recursive-search-deployedsystems
target:: /req/subdeployment/recursive-search-deployedsystems

test-purpose:: Verify that nested deployed systems can be queried using the `recursive` query parameter.

test-method:: Given the ID `depId` of a parent deployment that has subdeployments:
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/deployedsystems`.
step::: Verify that only systems deployed in the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/deployedsystems?recursive=false`.
step::: Verify that only systems deployed in the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/deployedsystems?recursive=true`.
step::: Verify that all systems deployed in the parent deployment and all its subdeployments (at all nesting levels) are included in the response.
====

[abstract_test]
====
[%metadata]
identifier:: /conf/subdeployment/recursive-search-fois
target:: /req/subdeployment/recursive-search-fois

test-purpose:: Verify that features of interest can be queried using the `recursive` query parameter.

test-method:: Given the ID `depId` of a parent deployment that has subdeployments:
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/featuresOfInterest`.
step::: Verify that only features of interest of the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/featuresOfInterest?recursive=false`.
step::: Verify that only features of interest of the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/featuresOfInterest?recursive=true`.
step::: Verify that features of interest of the parent deployment and all its subdeployments (at all nesting levels) are included in the response.
====

[abstract_test]
====
[%metadata]
identifier:: /conf/subdeployment/recursive-search-datastreams
target:: /req/subdeployment/recursive-search-datastreams

test-purpose:: Verify that nested datastreams can be queried using the `recursive` query parameter.

test-method:: Given the ID `depId` of a parent deployment that has subdeployments:
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/datastreams`.
step::: Verify that only datastreams associated to the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/datastreams?recursive=false`.
step::: Verify that only datastreams associated to the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/datastreams?recursive=true`.
step::: Verify that datastreams associated to the parent deployment and all its subdeployments (at all nesting levels) are included in the response.
====

[abstract_test]
====
[%metadata]
identifier:: /conf/subdeployment/recursive-search-controlstreams
target:: /req/subdeployment/recursive-search-controlstreams

test-purpose:: Verify that nested control streams can be queried using the `recursive` query parameter.

test-method:: Given the ID `depId` of a parent deployment that has subdeployments:
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/controlstreams`.
step::: Verify that only control streams associated to the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/controlstreams?recursive=false`.
step::: Verify that only control streams associated to the parent deployment are included in the response.
step::: Issue an HTTP GET request to the URL `{api_root}/deployments/{depId}/controlstreams?recursive=true`.
step::: Verify that control streams associated to the parent deployment and all its subdeployments (at all nesting levels) are included in the response.
====
