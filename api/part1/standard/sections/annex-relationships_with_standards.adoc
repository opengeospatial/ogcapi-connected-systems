...
<<OGC-OM,OGC Observations \&amp; Measurements (O\&amp;M)>> is one of the foundational building blocks of OGC API - Connected Systems, through our use of the {ogc-w3c-ssn}. Indeed, SOSA/SSN was originally based on O&M so all of the O&M concepts are naturally carried over to the API.

The new version of O&M now called <<OGC-OMS,Observations, Measurements and Samples (OMS)>> is also in scope as OGC API - Connected Systems is an implementation of several classes defined by this OGC/ISO Standard.
...
Part 2 of this Standard defines cross-service weblinking conventions to achieve this linkage in an interoperable manner.
...
Some resources defined by this API are designed to link to external feature resources that act as features of interest (e.g. rivers, roads, buildings, etc.). These features of interest can typically be hosted by separate instances of OGC API - Features.
