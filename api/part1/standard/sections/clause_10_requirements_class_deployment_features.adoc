...
| `deployedSystems`     | `sosa:deployedSystem` | The list of `Systems` deployed during the `Deployment`, if any. | A collection of `System` resources (inline or by-reference). | Required
...
| `samplingFeatures`    | - | The `Sampling Features` associated to `Systems` deployed during the `Deployment`. | A collection of `Sampling Feature` resources (inline or by-reference). | Optional
...
Any number of feature collections containing `Deployment` features can be available at a CS API endpoint, but the server must at least expose one. `Deployment` feature collections are identified with the feature type `sosa:Deployment`.
