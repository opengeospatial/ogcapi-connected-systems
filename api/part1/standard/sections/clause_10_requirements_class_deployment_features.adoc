...
| `platform`            | `ssn:deployedOnPlatform` | The platform on which the systems are deployed. | A single `Feature` resource (inline or by-reference). | Optional
...
Any number of feature collections containing `Deployment` features can be available at a CS API endpoint, but the server must at least expose one. `Deployment` feature collections are identified with the feature type `ssn:Deployment`.
