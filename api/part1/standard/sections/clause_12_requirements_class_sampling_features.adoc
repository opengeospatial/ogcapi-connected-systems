...
| `sampledFeature`          | The ultimate feature of interest that is being sampled or controlled. | A single `Feature` resource. | Required
| `relatedSamplingFeatures` | Other `Sampling Features` related to this `Sampling Feature` via sub-sampling. | A collection of `Sampling Feature` resources (inline or by-reference). | Optional
