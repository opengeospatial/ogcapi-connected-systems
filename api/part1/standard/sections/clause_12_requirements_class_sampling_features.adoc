...
A root collection where all `Sampling Feature` resources can be searched must be available at the API endpoint.
...
`Sampling Feature` collections are identified with the feature type `sosa:Sample`.
