...
In this API, `Sampling Feature` resources are a special kind of feature resource that implements the _http://www.w3.org/ns/sosa/Sample[sosa:Sample]_ concept.
...
include::../requirements/sampling/common/req_resource.adoc[]

==== Common Attributes

The following table lists attributes that are common to all `Sampling Feature` resources and their mapping to SOSA/SSN.
...
The following table lists associations that are common to all `Sampling Feature` resources and their mapping to SOSA/SSN.
...
| `sampledFeature`          | `sosa:isSampleOf` | he ultimate feature of interest that is being sampled or controlled. | A single `Feature` resource. | Required
