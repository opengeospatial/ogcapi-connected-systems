...
This standard requires that a `Property` resource has a canonical URL through which it can be retrieved independently of its membership to any collection.
...
A root collection where all `Property` resources can be searched must be available at the API endpoint.
