...
| `relatedSamplingFeatures` | `sosa:isSampleOf` | Other `Sampling Features` related to this `Sampling Feature` via sub-sampling. | A list of `Sampling Feature` resources (inline or by-reference). | Optional
...
part:: The server SHALL expose a {sf-resources-endpoint} at the paths defined by the URL template `{api_root}/systems/{sysId}/samplingFeatures`.
