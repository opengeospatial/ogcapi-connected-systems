...
Any number of feature collections containing `Sampling Features` can be available at a CS API endpoint, but the server must at least expose one. `Sampling Feature` collections are identified with the feature type `sosa:Sample`.
