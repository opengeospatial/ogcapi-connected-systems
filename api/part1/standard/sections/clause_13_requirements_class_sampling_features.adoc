...
The CS API Standard requires that a `Sampling Feature` resource has a canonical URL through which it can be retrieved independently of its membership to any collection.
...
A root collection where all `Sampling Feature` resources can be searched must be available at a CS API endpoint.
...
In addition to the default (mandatory) collection containing all sampling features, `Sampling Feature` resources can also be grouped into collections according to any arbitrary criteria. Any number of feature collections containing `Sampling Features` can be available at a CS API endpoint.
