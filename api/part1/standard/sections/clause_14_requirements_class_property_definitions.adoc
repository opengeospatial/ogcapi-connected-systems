...
Any number of resource collections containing `Property` resources can be available at a CS API endpoint, but the server must at least expose one. `Property` resource collections are identified with the item type `sosa:Property`.
