...
This requirements class specifies how instances of the resource types defined previously are created, replaced and deleted using the API.
...
{ogcapi-features-4} uses the terms "resources endpoint" and "resource endpoint" to identify the paths where these operations are supported by the server. The following sections provide these endpoints for each resource type defined by this Standard.
...
part:: For `Deployed System` resources, the resources endpoints used for CREATE operations shall be the URIs specified by the following URI templates:
  - `{api_root}/deployments/{depId}/systems`

part:: For `Deployed System` resources, the resource endpoints used for REPLACE and DELETE operations shall be the URIs specified by the following URI templates:
...
part:: When a resource defined in this standard is successfully created by the server at any endpoint, the server SHALL always make it available in the root collection corresponding to the resource type.
====
