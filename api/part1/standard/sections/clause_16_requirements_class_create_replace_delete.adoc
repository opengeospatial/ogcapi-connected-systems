...
part:: If the `System` resource is associated with a `Deployment` resource, the `Deployment` resource SHALL be be updated, removing the link to the System. If after updating there are no other linked systems in the `Deployment` resource, the `Deployment` SHALL be deleted.
====
