...
{ogcapi-features-4} uses the terms "resource endpoint" to identify the paths where the UPDATE operation is supported by the server. The following sections provide these endpoints for each resource type defined by this Standard.
