...
This requirements class specifies how `Feature` descriptions provided through the API are encoded using the <<OGC-SML-JSON, SensorML JSON>> format.
...
| `deploymentType` | `definition`  | Use the URI or CURIE from <<deployment-types>> as the value.
| `validTime`      | `validTime`   | -
...
| `deployedSystems`     | `deployedSystems`  | Value is a JSON Array of `DeployedSystem` objects.
...
[[clause-sensorml-deployed-system]]
==== Deployed System Representation

[requirement,model=ogc]
====
[%metadata]
identifier:: /req/sensorml/deployed-system-schema

conditions:: The server implements <<clause-deployment-features,/req/deployment-features>>.

part:: A request or response body with media type `{sensorml-json-mediatype}` containing a single `Deployed System` resource SHALL be valid against the JSON schema {sensorml-deployed-system-schema}.

part:: A request or response body with media type `{sensorml-json-mediatype}` containing a collection of `Deployed System` resources SHALL be valid against the JSON schema {sensorml-deployed-system-collection-schema}.
====

[requirement,model=ogc]
====
[%metadata]
identifier:: /req/sensorml/deployed-system-mappings

A SensorML JSON document representing a `Deployed System` resource SHALL implement the mappings specified in <<dep-sys-assoc-sml-mappings>>.
====

[#dep-sys-assoc-sml-mappings,reftext='{table-caption} {counter:table-num}']
.SensorML Mappings of Deployed System Associations
[width="100%",cols="3,3,10",options="header"]
|====
| *Association Name (see <<deployed-system-assocs>>)* | *JSON Member* | *Usage*
| `system`           | `system`      | Value is a weblink resolving to a single `System` resource.
| `procedure`        | `procedure`   | Value is a weblink resolving to a single `Procedure` resource.
| `payloads`         | `payloads`    | Value is a JSON Array of nested `Deployed System` resources.
|====

[[example_sensorml_depsys1]]
.A Deployed System resource in SensorML format
=================
[source%autofit,json]
----
include::../openapi/examples/deployments/deployedSystems/deployed-system-sml.json[]
----
=================
