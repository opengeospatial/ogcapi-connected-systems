...
.Link Relation Types
[width="100%",cols="3,5,10",options="header"]
|====
| *Relation Type*            | *Used in Resource*                          | *Description*
| `parentSystem`             | <<subsystem-assocs,`System (Subsystem)`>>, 
                               <<sf-assocs,`Sampling Feature`>>            | Link to the parent system of the entity.
| `subsystems`               | <<system-assocs,`System`>>                  | Link to the subsystems of a parent system.
| `samplingFeatures`         | <<system-assocs,`System`>>, 
                               <<deployment-assocs,`Deployment`>>          | Link to the sampling features associated to the entity.
| `deployments`              | <<system-assocs,`System`>>                  | Link to the deployments associated to the entity.
| `procedures`               | <<system-assocs,`System`>>                  | Link to the procedures that can be implemented by a system.
| `parentDeployment`         | <<subdeployment-assocs,`Deployment (Subdeployment)`>> | Link to the parent deployment of a subdeployment.
| `subdeployments`           | <<deployment-assocs,`Deployment`>>          | Link to the subdeployments of a parent deployment.
| `featuresOfInterest`       | <<system-assocs,`System`>>,
                               <<deployment-assocs,`Deployment`>>          | Link to the ultimate features of interest associated to the entity.
| `implementingSystems`      | <<procedure-assocs,`Deployment`>>           | Link to the systems that implement the procedure.
| `sampledFeature`           | <<sf-assocs,`Sampling Feature`>>            | Link to the the ultimate feature of interest sampled by the sampling feature.
| `relatedSamplingFeatures`  | <<sf-assocs,`Sampling Feature`>>            | Link to other sampling features that are either parent sample or subsamples of the sampling feature.
| `datastreams`              | <<system-assocs,`System`>>,
                               <<deployment-assocs,`Deployment`>>,
                               <<sf-assocs,`Sampling Feature`>>            | Link to the datastreams that are associated to the entity,
| `controlStreams`           | <<system-assocs,`System`>>,
                               <<deployment-assocs,`Deployment`>>,
