...
NOTE: `System` resources describe system instances (e.g. a sensor device, a UAV platform, a human observer) while `Procedure` resources describe types of systems (e.g. a particular model of sensor or vehicle, a well-defined procedure followed by a human operator, etc.). Several `System` instances can implement the same `Procedure` (i.e. be of the same model).
...
| `procedure`         | Link to the `Procedure` implemented by the `System` | A single `Procedure` resource. | Required
| `components`        | Link to the components of the `System` (i.e. subsystems), if any  | A collection of `System` resources. | Required
| `samplingFeatures`  | Link to the `Sampling Features` associated to the `System` | A collection of `SamplingFeature` resources. | Required
| `deployments`       | Link to the `Deployments` that the `System` is part of, if any | A collection of `Deployment` resources. | Optional
|====
