...
NOTE: `System` resources are used to model all subclasses of _http://www.w3.org/ns/ssn/System[ssn:System]_, including _http://www.w3.org/ns/sosa/Sensor[sosa:Sensor]_, _http://www.w3.org/ns/sosa/Actuator[sosa:Actuator]_, _http://www.w3.org/ns/sosa/Sampler[sosa:Sampler]_, as well as _http://www.w3.org/ns/sosa/Platform[sosa:Platform]_. Semantic tagging is used to provide the exact system type.
...
In this API, `System` resources are a special kind of feature resource that implements the _http://www.w3.org/ns/ssn/System[ssn:System]_ concept.
...
include::../requirements/system/req_resource.adoc[]

==== Attributes

The following table lists `System` resource attributes and their mapping to SOSA/SSN.
...
The following table lists `System` resource assocations and their mapping to SOSA/SSN.
...
| `procedure`         | `ssn:implements` | The `Procedure` implemented by the `System`. | A single `Procedure` resource. | Required
| `subsystems`        | `ssn:hasSubSystem` | The components of the `System`, if any. | A collection of `System` resources (inline or by-reference). | Required
...
| `datastreams`       | - | The `DataStreams` generated by the `System`, if any. | A collection of `DataStream` resources (inline or by-reference). | Optional
| `controlstreams`    | - | The `ControlStreams` receiving commands for the `System`, if any. | A collection of `ControlStream` resources (inline or by-reference). | Optional
| `deployments`       | `ssn:hasDeployment` | The `Deployments` that the `System` is part of, if any. | A collection of `Deployment` resources (inline or by-reference). | Optional
|====

==== Location

It is recommended that the `System` resource representation includes the location of the system. If the implementation decides to report the location, it must be done according to the optional `datetime` request parameter.
