...
Likewise, when listing a system's sub-resources (i.e. subsystems, sampling features, datastreams, control streams), only the resources directly associated to the system are included in the response, but not the ones associated to subsystems that are farther down the tree.

The `recursive` query parameter changes the default behavior by directing the server to recurse through the system tree to process all subsystems (at all levels below the current level).
