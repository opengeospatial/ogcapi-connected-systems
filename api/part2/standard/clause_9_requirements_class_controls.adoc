...
| `validTime`            | The validity period of the control stream's description. | TimePeriod | Optional
| `issueTime`            | The time period spanned by the issue times of all commands in this datastream. | TimePeriod | Mandatory
| `executionTime`        | The time period spanned by the execution times of all commands in this datastream. | TimePeriod | Mandatory
| `controlledProperties` | Properties whose value can be changed by commands in this control stream. | List<URI> | Mandatory
| `formats`              | The list of formats that the commands in this datastream can be encoded to. | List<String> | Mandatory
|====
...
Note that a different schema is needed for each individual control stream because the exact content of the command `paramters` property changes according to the properties being controlled.
...
`Command` resources exposed through the API are always part of a `CommandStream`, thus they don't have to include all mandatory properties as many of them can be factored at the command stream level.
...
| `executionTime`      | The time period during which the command was executed. The time period ends when the effect of the command has modified all controlled properties of the feature of interest. | TimePeriod | Optional
