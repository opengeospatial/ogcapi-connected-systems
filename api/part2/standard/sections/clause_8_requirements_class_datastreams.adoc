...
- <<clause-json-datastream,DataStream resource as JSON>>
...
.DataStream Resource Diagram #TODO Update diagram#
...
| `phenomenonTime`     | `sosa:phenomenonTime` | The time period spanned by the phenomenon times of all observations in this datastream. | TimeExtent | Required
| `resultTime`         | `sosa:resultTime` | The time period spanned by the result times of all observations in this datastream. | TimeExtent | Required
| `observedProperties` | `sosa:observedProperty` | Properties for which the observations in this datastream provide measurements. | List<URI> | Required
| `resultType`         | - | The type of result for observations in this datastream (see <<result-types>>). | Enum | Required
| `live`               | - | Indicates whether live data is available from this datastream. | Boolean | Required
| `formats`            | - | The list of formats that the observations in this datastream can be encoded to. | List<String> | Required
|====
...
| `observation`       | For datastreams providing observations of other features of interest.
|====
...
| `system`             | `sosa:isObservedBy` | The `System` that is the producer of this datastream. | A single `System` resource (by reference). | Required
| `observations`       | `sosa:hasMember` | The `Observations` that are part of this datastream. | A list of `Observation` resources. | Required
| `samplingFeatures`   | `sosa:hasFeatureOfInterest` | The `Sampling Features` that are the subject of observations in this datastream. | A list of `SamplingFeature` resources. | Optional
| `featuresOfInterest` | `sosa:hasUltimateFeatureOfInterest` | The ultimate features of interest that are the subject of observations in this datastream. | A list of `Feature` resources. | Optional
|====
...
part:: The endpoint SHALL only expose the `Feature` resources that are associated to observations in the parent `DataStream` with ID `dsId`.
====
...
NOTE: A different schema is needed for each individual datastream because the exact content of the observations result changes according to the type of sensor and the properties being observed.

NOTE: The exact content of a schema resource is defined by each encoding. See section <<clause-encoding-json>> for example schemas used for observations encoded using the default JSON format.
...
|====
| *Name*              | *SOSA/SSN Property* | *Definition* | *Data Type* | *Usage*
| `phenomenonTime`    | `sosa:phenomenonTime` | The time the measured property value applies to the feature of interest. | DateTime | Required
...
include::../requirements/observation/req_canonical_url.adoc[]
...
include::../requirements/observation/req_resources_endpoint.adoc[]

==== Basic Observation Filters

An Observation resources endpoint supports filtering using the query parameters described below.

<<clause-observation-query-params>> defines additional query parameters applicable to `Observation` resources endpoint.
...
include::../requirements/observation/req_canonical_endpoint.adoc[]
...
include::../requirements/observation/req_collections.adoc[]

#TODO Add properties to collection metadata (ultimateFeatureOfInterest, observedBySensor, etc.)#
