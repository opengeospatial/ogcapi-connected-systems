...
Thus, for each `DataStream` resource, the CS API provides a way for the server to communicate an observation schema (_not necessarily a JSON schema_) for each supported observation format. The exact content of a schema resource is defined by each encoding. See section <<clause-encoding-json>> for example schemas used for observations encoded using the default JSON format.
