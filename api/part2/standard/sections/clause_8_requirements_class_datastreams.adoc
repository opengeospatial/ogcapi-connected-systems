...
A different schema is needed for each individual datastream because the exact content of the observations result changes according to result type and the properties being observed. Moreover, multiple observation formats can be offered for any given datastream.

The CS API thus provides a way for the server to communicate the schema (_not necessarily a JSON schema_) corresponding to a given datastream and observation format. The exact content of a schema resource is defined by each encoding. See section <<clause-encoding-json>> for example schemas used for observations encoded using the default JSON format.
