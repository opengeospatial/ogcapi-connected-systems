...
Thus, for each `ControlStream` resource, the CS API provides a way for the server to communicate a command schema (_not necessarily a JSON schema_) for each supported command format. The exact content of a schema resource is defined by each encoding. See section <<clause-encoding-json>> for example schemas used for commands encoded using the default JSON format.
