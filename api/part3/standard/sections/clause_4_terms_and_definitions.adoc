...
NOTE: Stability concerns identity, not network location or availability. A stable URI is retained across publisher restarts, broker migrations, and channel-address changes. It continues to identify a deleted resource in historical events, even when no representation remains available at that URL.

NOTE: In this Standard, event source, resource, and collection identifiers use the absolute API root and canonical URLs provided by the associated CS API HTTP server. This definition does not introduce an alternative URI scheme for those attributes.
...
=== Format Name
An identifier for a resource encoding used by a protocol binding to distinguish pub/sub channels carrying different representations of the same resource. An optional normalization convention is defined in /req/resource-data-messages/content-negotiation/format-name.