...
Every parametric sampling feature has a `pose` attribute that takes an instance of a `Pose` object based on the <<OGC-SML,SensorML>> pose model, itself derived from the <<GeoPose,GeoPose>> model.
...
The allowed values for the `pose` attribute are "GeoPose Basic" instances or "<<OGC-SML,SensorML Relative Pose>>" instances.
