...
The “UnitReference” object is the JSON schema implementation of the “UnitOfMeasure” UML classes used in <<uml_quantity_class>> and <<uml_quantityrange_class>>,  and the “UomTime” UML class used in <<uml_time_class>> and <<uml_timerange_class>>. The schema for this class is provided in {jsonschema-base-url}/basicTypes.json[basicTypes.json] (see #definitions/UnitReference). Further details on the usage of “UnitReference” object can be found in <<json_quantity_obj>> and <<json_time_obj>>.

Examples of references to an unit of measure are provided below:
