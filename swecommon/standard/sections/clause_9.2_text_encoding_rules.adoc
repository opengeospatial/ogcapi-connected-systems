...
The following snippet is an example of datastream where each element represents a feature detected in a video stream, and geolocated. Each record is composed of a timestamp, a scalar field and a geometry: 

[[enc_geometry_example]]
[source%unnumbered,xml]
----
<swe:DataStream>
...
- `application/swe+text` shall be used when a client requests this format from an API or web service.
