...
When array or stream values are encoded with the JSON encoding method and provided standalone (i.e. outside of any wrapper format), the `application/swe+json` identifier shall be used whenever media type information is needed by the application.